class PSCalculatorController {
    ...
    onDocumentClick(event) {
        ...
        this.view.gpuBlocks.forEach((_, index) => {
    ...
    onAddGpu() {
        this.gpuCount++;
        this.view.addGpuBlock(this.gpuCount, window.gpuOptionsHtml, window.STATIC_URL);
    ...
    onDeleteGpu(index) {
        console.log('Deleting GPU:', index);
        this.model.removeGpu(index);
        if (this.view.gpuBlocks[index]) {
            this.view.gpuBlocks[index].remove();
            this.view.gpuBlocks.splice(index, 1);
            this.gpuCount--;
            this.updatePower();
        }
    }
